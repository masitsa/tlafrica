...
var url = config_url+"admin/charts/donations_totals/"+current_timestamp;//alert(url);
...
for(r = 0; r < 8; r++)
{
	$.ajax({
		...
		success:function(data){
			...
			pledged.push([current_timestamp, data.pledged]);
			paid.push([current_timestamp, data.paid]);
			
			current_timestamp = current_timestamp - 86400000;
			url = config_url+"admin/charts/donations_totals/"+current_timestamp;
		},
		error: function(xhr, status, error) {
			//alert(url);
			alert("XMLHttpRequest=" + xhr.responseText + "\ntextStatus=" + status + "\nerrorThrown=" + error);
	...
	var plot = $.plot($("#curve-chart"), 
		...
		
		{
		   series: {
			...
			yaxis: {axisLabel: "Total Donations"},
...
$.ajax({
	type:'POST',
	url: config_url+"admin/charts/hills_totals",
	cache:false,
	contentType: false,
	processData: false,
	dataType: "json",
	success:function(data){
		...
		var total_hills = data.total_hills;
		...
		$(function () {
			...
			var d3 = [];
			var ticks = [];
			for(r = 0; r < parseInt(total_hills); r += 1)
			{
			...
			function plotWithOptions() {
				$.plot($("#bar-chart"), [ d3 ], {
					...
					xaxis: {axisLabel: "Hills", ticks: ticks},
					yaxis: {axisLabel: "Total Donated", 
