...
class Charts extends admin {
	...
	function donations_totals($timestamp)
	{
		$date = gmdate("Y-m-d", ($timestamp/1000));
		
		//get all patient types
		$status_result = $this->reports_model->get_all_donation_types();
		...
		if($status_result->num_rows() > 0)
		{
			...
			foreach($result as $res)
			{				
				$donation_type_id = $res->donation_type_id;
				$donation_type_name = $res->donation_type_name;
				
				//get method total
				$total = $this->reports_model->get_donations_total($donation_type_id, $date);
				...
				$result[strtolower($donation_type_name)] = $total;
			}
		}
	...
	function hills_totals()
	{
		$this->load->model('hills_model');
		//get all patient types
		$parents = $this->hills_model->all_active_hills();
		...
		if($parents->num_rows() > 0)
		{
			...
			{
				$hill_id = $res->hill_id;
				$hill_name = $res->hill_name;
				
				//get service total
				$total = $this->reports_model->get_hills_total($hill_id);
		...
		$result['total_hills'] = $parents->num_rows();
		$result['names'] = $names;
		$result['bars'] = $totals;
		$result['highest_bar'] = $highest_bar;
		echo json_encode($result);
